...
export class vueAnimatedProps {
    ...
    coolwarmReds: number[]
    coolwarmBlues: number[]
    coolwarmGreens: number[]
    windParticleTextureData: { width: number; height: any; type: number; data: any; format: number; dataFormat: number; parameters: { [x: number]: number }; pixelStore: { [x: number]: boolean } }
    windParticleTextureDataTempHumidity: { width: number; height: any; type: number; data: any; format: number; dataFormat: number; parameters: { [x: number]: number }; pixelStore: { [x: number]: boolean } }
    windParticleBounds: any[]

    constructor(fireID: string){
        ...
        perimeter_data_raw.slice(0, -1).forEach( (d,index) => {
            let dateObj = new Date(d.perimeter_date)

            this.perimeterData.push({
                path: d.perimeter,
                startTime: this.timeScale(dateObj),
                endTime: this.timeScale(new Date(perimeter_data_raw[index + 1].perimeter_date)),
                timeString: dateObj.toLocaleString('ja-JP').slice(5, 9),
                time: dateObj
            })
        })

        this.smoothPerimeter(this.perimeterData)
        ...
        this.frpData = formatted_frp_data
    ...
    smoothPerimeter(data){
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            let timeWindow: number = -1.66 * temp * temp + 20.02 * temp + 6.64
            timeWindow = Math.round(timeWindow)
            ...
            for (let i = 0; i < currentPerimeter.length; i++){
                ...
                data[index].smoothed_path[i] = [x_temp / (timeWindow*2+1),  y_temp / (timeWindow*2+1)]
            }
    ...
    formattedHeatMapCollection(frp_data: any[], timeScale){
        let heatmapdata = []
        ...
        let colorScale_gridcell = d3.scaleLinear<string, string>().domain(d3.extent(frp_data.map(d=>parseFloat(d.power)))).range(["white", "red"])
    ...
    prepareWindParticleLayer(){
        ...
        //
        let textureData_Data = []
        let textureDataTempHumidity_Data = []
        ...
        for (let row = 0; row < windFieldHeight; row++) {
            for (let windFieldIndex = 0; windFieldIndex < numWindFields; windFieldIndex++){
                for (let col = 0; col < windFieldWidth; col++) {
                    ...
                    textureData_Data.push(Math.sin((windData[windFieldIndex].grids.wind_direction[index]) * (Math.PI / 180))*curWindScale*128+128)
                    textureData_Data.push(Math.cos((windData[windFieldIndex].grids.wind_direction[index]) * (Math.PI / 180))*curWindScale*128+128)
                    // textureData.data.push(temperatureScale(windData[windFieldIndex].grids.air_temp[index]));
                    textureData_Data.push(0)
                    textureData_Data.push(255)

                    textureDataTempHumidity_Data.push(temperatureScale(windData[windFieldIndex].grids.air_temp[index]))
                    textureDataTempHumidity_Data.push(humidityScale(windData[windFieldIndex].grids.relative_humidity[index]))
                    textureDataTempHumidity_Data.push(0)
                    textureDataTempHumidity_Data.push(255)
                }
            }
        }
        ...
        */
        textureData.data = new Uint8Array([...textureData_Data])
        textureDataTempHumidity.data = new Uint8Array([...textureDataTempHumidity_Data])
