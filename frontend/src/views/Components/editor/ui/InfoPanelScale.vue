<template>
    <h3>{{ props.scale.variable }}</h3> 
    <div :style="{'align-items': 'center'}">
        <div
            v-for="(color, index) in props.scale.colorRange"
            :key="index"
            class="InfoPanelScale-legend"
    ...
    <div
        ...
        :key="index"
        class="InfoPanelScale-labels"
        :style="{'width': widthLabels}"
    >
        <div>{{ props.scale.labels[index] }}</div>
    </div>
</template>
...
const props = defineProps<{
    scale: InfoPanelProps.InfoPanelScale
}>()

const colorRamp = props.scale.colorRange.map(color => `rgb(${color.join(',')})`)
